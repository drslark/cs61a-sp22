...
def next_perfect_square(score):
    ...
    assert i == d, "score must be a perfect square."
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    num_rolls = 1
    while num_rolls <= 10:
        score = averaged_roll_dice(num_rolls, dice)
        if score > best_score:
...
SIX_ROLLS_SCORE = make_averaged(roll_dice, 10000)(6)


def final_strategy(score, opponent_score):
    """Write a brief description of your final strategy.

    This strategy returns 0 dice when new score is greater than GOAL
    or your score would increase by at least average six rolls score,
    returns number of rolls smaller than 6 when it has the max probability,
    returns 5 when score is smaller than opponent's score.
    Otherwise, returns 6.
    """
    # BEGIN PROBLEM 12
    new_score = square_update(0, score, opponent_score)
    if new_score >= GOAL or new_score - score >= SIX_ROLLS_SCORE:
        return 0
...
def print_max_prob_rolls_table(max_points):
    """Prints the table of points and its max probability rolls."""
    points = 2
    while points <= max_points:
        print("points: ", points, " max prob rolls: ", get_six_sided_max_prob_rolls(points))
        points += 1


def get_six_sided_max_prob_rolls(points):
    """Returns the number of rolls to get a max probability
    of rolling a six-sided dice to get the specified points.
    """
    num_rolls = 1
    curr_prob = compute_six_sided_cum_prob(points, num_rolls)
    next_prob = compute_six_sided_cum_prob(points, num_rolls + 1)
    while curr_prob <= next_prob:
        num_rolls += 1
        curr_prob = next_prob
        next_prob = compute_six_sided_cum_prob(points, num_rolls + 1)
...
def compute_six_sided_cum_prob(points, num_rolls):
    """Computes the probability of rolling a six-sided dice to get points bigger than the specified points."""
    assert int(points) == points, "points must be an integer."
    assert points > 1, "points must be at least 2."

    all_poss = pow(6, num_rolls)
    prob = pow(5 / 6, num_rolls)

    curr_points = 2
    while curr_points < points:
        prob -= compute_six_sided_counts(curr_points, num_rolls) / all_poss
        curr_points += 1

    return round(prob, 5)


def compute_six_sided_counts(points, num_rolls):
    """Computes the counts of rolling a six-sided dice to get the specified points."""
    assert int(points) == points, "points must be an integer."
    assert points > 1, "points must be at least 2."

    if num_rolls == 0 and points == 0:
        return 1
    if points <= 0 or num_rolls <= 0:
        return 0
    ...
    i = 2
    while i <= 6:
        counts += compute_six_sided_counts(points - i, num_rolls - 1)
        i += 1
